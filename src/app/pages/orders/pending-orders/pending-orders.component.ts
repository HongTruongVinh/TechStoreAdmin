...
export class PendingOrdersComponent {
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbItems = [
      ...
      { label: 'Các đơn đặt hàng mới', active: true }
    ];
  ...
  loadData() {
    this.isLoading = true;
    this.orderService.getOrdersByStatus(EOrderStatus.Pending).subscribe((res) => {
      if (res.retCode == 0) {
        if (res.data) {
  ...
  acceptOrderAction() {
    ...
    if (id != null && id != "" && id != undefined) {
      this.orderService.processingOrder(id).subscribe((res) => {
        if (res.retCode == 0) {
          this.loadData();
